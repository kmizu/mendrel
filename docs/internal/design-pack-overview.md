...
The current parser subset accepts a module declaration; direct, grouped, and aliased import declarations; module-private, `pub`, and `internal` record declarations with ordered fields using qualified-name or `Self` types; module-private, `pub`, and `internal` enum declarations with ordered plain or record-style payload variants using the same field subset; and public function declarations with typed identifier parameters and an explicit return type. Function bodies may contain identifier-binding `let` statements with optional qualified-name or `Self` type annotations and `return` statements with or without a value, in source order, followed by an optional trailing expression when at least one statement is present. Expressions may use identifiers, integer, string, boolean, `Unit`, and `None` literals, the additive operators `+` and `-`, the multiplicative operators `*`, `/`, and `%`, grouping parentheses, and chained function calls. Record and enum payload fields may also be module-private, `pub`, or `internal`. Grouped imports accept item aliases and an optional trailing comma. Calls may have zero or more positional or named arguments and an optional trailing comma.

Declaration and field attributes, record and enum generic parameters and `where` clauses, type arguments and compound types, non-identifier binding patterns, `var`, `use`, and all statement forms other than `let` and `return`, qualified expression paths, unary operators, empty parameter lists, empty function bodies, omitted or `internal` function visibility, `async`, `unsafe`, and `move` parameters are outside the current slice. Return parsing is syntactic only: type compatibility, reachability, and control-flow semantics remain for later compiler phases. Literal lexical forms whose shapes or escape rules remain unspecified by the design pack are also outside this slice. Unsupported syntax is rejected with `E-SYNTAX-UNSUPPORTED-0001` rather than accepted with provisional semantics.
