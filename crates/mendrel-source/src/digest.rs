...
pub(crate) fn sha256(bytes: &[u8]) -> [u8; 32] {
    ...
    for chunk in &mut chunks {
        compress(
            &mut state,
            chunk.try_into().expect("SHA-256 chunk is 64 bytes"),
        );
    }
    ...
    let bit_length = u64::try_from(bytes.len())
        ...
        .wrapping_mul(8)
        .to_be_bytes();
    tail[padded_length - 8..padded_length].copy_from_slice(&bit_length);
    for chunk in tail[..padded_length].chunks_exact(64) {
        compress(
            &mut state,
            chunk.try_into().expect("SHA-256 chunk is 64 bytes"),
        );
...
pub(crate) fn revision(bytes: &[u8]) -> String {
    ...
    for byte in sha256(bytes) {
        write!(revision, "{byte:02x}").expect("writing to String cannot fail");
    }
    revision
}
...
fn compress(state: &mut [u32; 8], block: &[u8; 64]) {
    ...
    for (index, bytes) in block.chunks_exact(4).enumerate() {
        schedule[index] = u32::from_be_bytes(bytes.try_into().expect("word is four bytes"));
    }
    for index in 16..64 {
    ...
    for (constant, word) in ROUND_CONSTANTS.into_iter().zip(schedule) {
        ...
        let temporary_2 = upper_a.wrapping_add(majority);
